setTimeout(languages, 0);

function languages() {
    console.log(true);
    const xhr = new XMLHttpRequest();
    xhr.open('GET', "https://github-trending-api.now.sh/developers?language=javascript&since=weekly", true);
    xhr.onload = function() {
        if (this.status == 200) {
            ...
            for (let i = 0; i < abba; i++) {
                if (i % 3 == 0) {
                    ...
                    style = "width:295px " >
                      ...
                      <h4 class = "card-title text-white" > ${languages[i].username} </h4> 
                      <p class = "card-text text-white" > ${languages[i].name}</p>
                ...
                if (i % 3 == 1) {
                    ...
                    style = "width:295px" >
                      ...
                      <h4 class = "card-title  text-white" > ${languages[i].username} </h4> 
                      <p class = "card-text text-white" > ${languages[i].name} </p>
                      <a href = "${languages[i].url}" class = "btn btn-primary">Profile</a> 
                      </div> 
                      </div>
